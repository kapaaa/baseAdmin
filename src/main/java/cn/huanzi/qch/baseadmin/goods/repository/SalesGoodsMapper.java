...
public interface SalesGoodsMapper extends BaseMapper<SalesGoods> {
    ...
    @Select("SELECT  b.id,a.id as goods_id,a.name,IFNULL(b.sale_num,0) AS sale_num,a.img FROM goods a LEFT JOIN goods_sales b ON a.id=b.goods_id AND b.create_time>=#{time} ${sql} ")
    List<SalesGoodsVo> salesGoodsList(@Param("sql")String sql, @Param("time") String time);

    Integer salesOne(@Param("id")String id,@Param("goodsId")String goodsId,@Param("saleNum")Integer saleNum,@Param("createTime")String createTime);
